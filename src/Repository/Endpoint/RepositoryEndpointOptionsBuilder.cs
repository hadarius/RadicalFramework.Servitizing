...
{
    public static class RepositoryEndpointOptionsBuilder
    {
        public static IServiceRegistry AddEntityServicesForDb(EndpointProvider provider)
        {
            ...
            if (!DataStoreRegistry.Providers.ContainsKey((int)provider))
            {
                switch (provider)
                {
                ...
                DataStoreRegistry.Providers.Add((int)provider, provider);
            }
            return registry;
        }
