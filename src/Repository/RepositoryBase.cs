...
public abstract class Repository : IRepository
{
    ...
    public string Name => ElementType.GetProxyEntityName();

    public string Fullname => ElementType.GetProxyEntityFullName();
    ...
    {
        get =>
            serialcode.TypeKey == 0
                ? serialcode.TypeKey = ContextType.UniqueKey32()
                : serialcode.TypeKey;
    ...
    public virtual void LinkTrigger(object sender, EntityEntryEventArgs e)
    {
        ...
        var type = entity.GetProxyEntityType();

        if (type.IsAssignableTo(typeof(IEntity)) && type == ElementType)
        {
            LinkedObjects.DoEach(
                async (o) =>
                {
    ...
    public virtual object TracePatching(
        object item,
    ...
    protected abstract Task<int> saveAsTransaction(
        CancellationToken token = default
    );

    protected abstract Task<int> saveChanges(
        CancellationToken token = default
    );
}
