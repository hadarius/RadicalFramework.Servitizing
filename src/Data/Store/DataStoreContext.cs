...
public class DataStoreContext<TStore> : DataStoreContext, IDataStoreContext<TStore> where TStore : IDatabaseStore
{
    protected virtual Type StoreType { get; }

    public DataStoreContext(DbContextOptions options, IServicer servicer = null) : base(options, servicer)
    {
        ...
        catch (DbUpdateException e)
        {
            if (e is DbUpdateConcurrencyException)
                ...
                tr.Failure<Datalog>(
                    ...
                    $" context Id:{ContextId}, TimeStamp:{DateTime.Now.ToString()} {e.StackTrace}", e.Entries);
